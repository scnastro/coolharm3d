...
static double adiab, rin, rpmax, GRscale, l_ang_in, q;
...
/***********************************************************************************
  ...
  ---------
   ...
   -- Initial data for a cicumbinary disk in an evolving spacetime determined by 
    ...
    -- The disk model is that of init.kd.c  or "KD" or Keplerian disk initial data 
          ala De Villiers and Hawley 2002-2004 B-field  follows density contours. 
...
void init(void)
{
  ...
  initial-data-is-inconsistent-with-METRIC_DYNAMIC_KS_SPHERICAL
#endif
...
void set_special_coord(void)
{
  ...
  Rin = 0.05 * (initial_bbh_separation) ; /*  put inner boundary at 2. times the binary orbit's radius */
  R0  = 0.;
...
void init_prim(int i, int j, int k, struct of_coord *coords, double *pr)
{
  ...
  **********************************************************************************************/  
  ...
  if( first_call ) { 
    ...
    rin      = 2. *(initial_bbh_separation+m_bh_tot); /* Radius of inner edge of the disk in BL coordinates */
    ...
    //    q        = 1.68;    /* exponent in definition of Omega  */
    q        = 1.58;    /* exponent in definition of Omega  */
