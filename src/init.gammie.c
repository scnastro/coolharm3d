...
/***********************************************************************************
  ...
  ---------
   ...
   Cold Equatorial Inflow in Kerr (aka Gammie Solution) 
...
void init_base(void)
{
	...
	dx[0] = 1.e-3;               /* Discretization size in X0 direction */
...
int init_dsandvels(int i, int j, int k, double *xp, double *pr)
{
  ...
  if( first_one ) { 
    ...
    if( (r_bondi = (double *) calloc(N1TOT, sizeof(double))) == NULL ) { 
      fprintf(stdout,"init_dsandvels: Cannot alloc r_bondi \n");
      return(1);
    }
    if( (rho_bondi = (double *) calloc(N1TOT, sizeof(double))) == NULL ) { 
      fprintf(stdout,"init_dsandvels: Cannot alloc rho_bondi \n");
      free(r_bondi);
      return(1);
    }
    if( (u_bondi = (double *) calloc(N1TOT, sizeof(double))) == NULL ) { 
      fprintf(stdout,"init_dsandvels: Cannot alloc u_bondi \n");
      free(r_bondi);      free(rho_bondi);
      return(1);
    }
    if( (v_bondi = (double *) calloc(N1TOT, sizeof(double))) == NULL ) { 
    ...
    for( iit = (ir_spt+1); iit < N1TOT; iit++ ) { 
  ...
  pr[ U1] = ucon_ks[RR] - gcov[TT][RR] * ucon_ks[TT] / gcov[TT][TT] ; 
  pr[ U2] = ucon_ks[TH] - gcov[TT][TH] * ucon_ks[TT] / gcov[TT][TT] ; 
  pr[ U3] = ucon_ks[PH] - gcov[TT][PH] * ucon_ks[TT] / gcov[TT][TT] ; 
  pr[ B1] = 0. ;
  pr[ B2] = 0. ;
  pr[ B3] = 0. ;
