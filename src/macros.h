...
#if(METRIC_TYPE_CHOICE==METRIC_MINK_CARTESIAN||METRIC_TYPE_CHOICE==METRIC_KS_CARTESIAN)
#define TOP_TYPE_CHOICE (TOP_CARTESIAN)
...
#if((METRIC_TYPE_CHOICE==METRIC_GENERAL_DYNAMIC)||(DYNAMIC_COORDINATES))
...
#define MASK_NORMAL   2    /* Regular evolution, everything is calculated */


/* Photospheric mnemonics: */
#define PHOTO_FLUX    (0)
#define PHOTO_TH_BOT  (1)
#define PHOTO_TH_TOP  (2)
#define N_PHOTO_FUNCS (3)
...
#define OUT_PHOTOSPHERE ( 8)
#define OUT_MIN_DT      ( 9)
#define OUT_SURFACE     (10)
...
//#define N_HDF_FUNCS (NP+4+NDIM+1)

/* MHD + gcov + gdet + pflag + nfail + evol_mask */ 
//#define N_HDF_FUNCS (NP+4+10+1+1+N_NFAIL+1)

/* MHD + gcov + gdet + pflag  */ 
#if( USE_KINEMATIC_VISCOSITY )
#define N_HDF_FUNCS (NP+4+10+1+1+4)
#else
...
#define N_SURFACE  (N_SURFACE_BASE+N_SURFACE_CURRENT+N_SURFACE_RADFLUX+N_SURFACE_DYNMET)
#define N_HISTORY  (N_HISTORY_BASE+N_HISTORY_CURRENT+N_HISTORY_RADFLUX+N_HISTORY_DYNMET)

/* Number of groups or subdirectories in the root group (e.g. Bound, Unbound...): */
#define UNBOUND  (0)
#define BOUND    (1)
#define JET      (2)
#define N_HIST_TYPES  (3)
...
#undef MAKE_PHOTOSPHERE
#define MAKE_PHOTOSPHERE (0)
#endif
...
/* Photosphere loops for corona2 related routines : */ 
#define PHOTO_N1_LOOP    for( ip=0; ip < n1_photo; ip++ ) 
#define PHOTO_N3_LOOP    for( kp=0; kp < n3_photo; kp++ ) 
#define PHOTO_N_LOOP     for( np=0; np <  n_photo; np++ ) 
#define PHOTOFUNC_LOOP   for( ifunc=0; ifunc < N_PHOTO_FUNCS; ifunc++ ) 
#define PHOTODATA_LOOP   for( ip=0; ip < n_data_photo; ip++ )

#define SMALL	  (1.e-15)   /* numerical convenience */
...
#if( TRACE_CALLS )
#define TRACE_BEG  { fprintf(stdout,"%s beg : %d %d %d \n",__func__,myid,nstep,n_substep); }
#define TRACE_END  { fprintf(stdout,"%s end : %d %d %d \n",__func__,myid,nstep,n_substep); }
#else 
#define TRACE_BEG {;}
#define TRACE_END {;}
#endif

#define DEBUG_OUT(_str_tag)  {fprintf(stdout,"%s : %s %s %10d : %10d %12d %6d \n",(_str_tag),__func__,__FILE__,__LINE__,myid,nstep,n_substep); fflush(stdout); }

#define ERROR_MESSAGE(_str_tag)  {fprintf(stdout,"%s : %s() line=[%10d] : myid=[%10d]  nstep=[%12d] n_substep=[%6d] : %s  \n",__FILE__,__func__,__LINE__,myid,nstep,n_substep,(_str_tag)); fflush(stdout); }
