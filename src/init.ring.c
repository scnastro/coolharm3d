...
/***********************************************************************************
 ...
 Exponential distribution of isentropic fluid in the shape of a ring lying in the 
  equator of the black hole. 
...
void init_prim(int i, int j, int k, double *xp, double *pr)
{
  ...
  /* for disk interior */
  double kappa, r,th,ph,x0,y0,z0,r0;
  ...
  /* The velocity of the fluid is at rest */ 
...
//  ucon2pr( pr, ucon, gcon[i][j][CENT] );
  ...
  /* The profile of the matter distribution is an isentropic Gaussian */ 
  x_of_xp( x, xp ); 
  r = x[RR];  th = x[TH]; ph = x[PH];
  x0 = r*sin(th);
  z0 = r*cos(th);

  pr[RHO] = exp( -((x0-r0)*(x0-r0) + z0*z0)/(del_blob*del_blob));
  pr[UU]  = pow( pr[RHO], gam ) / (gam - 1.);
