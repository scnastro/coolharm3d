...
#define BC_SPHERICAL_FULLSPHERE4 (21)     /* Spherical grid covers the spherical domain, sharing ghostzone data across axis and origin though setting v=0 at origin, no origin cell, only one outflow condition on the outer radial boundary */
...
#define BC_TYPE_CHOICE           (BC_SPHERICAL_OUTFLOW3)
...
#define COORD_TYPE_CHOICE  (COORD_DIAGONAL3)
...
/* whether to use the cooling function in the source terms:
   ...
   5  ->  like 4 but find urad by integrating photosphere's flux
 */
#define USE_COOLING_FUNCTION (5)   
...
#define EXCISION_TYPE_CHOICE (EXCISE_HORIZON)
 ...
 /**************** OUTPUT OPTIONS: ***********************/ 
#define MAKE_HISTORY     (1)  /* Whether to generate history    files */
...
#define MAKE_STAT        (1)  /* Whether to generate statistical information files (hdf only) */
#define MAKE_STAT2       (0)  /* Whether to dump left/right fluxes, U, P, sources, ctop (hdf only) */
#define MAKE_RADFLUX     (1)  /* Whether to make separate hdf5 file for radiative fluxes */
#define MAKE_PHOTOSPHERE (1)  /* Whether to make separate hdf5 file for photosphere data */
...
#define OUTPUT_MAX_VCHAR      (0) /* Whether to local fastest characteristic speeds used for the fluxes and timestep determination */
