...
******************************************************************/
int Utoprim_2d_fast(FTYPE *U, struct of_geom *geom, FTYPE *prim, FTYPE *gamma, FTYPE *bsq)
{
...
static int Utoprim_new_body(FTYPE *U, struct of_geom *geom, FTYPE *prim, FTYPE *gamma_out, FTYPE *bsq)
{
  ...
  FTYPE x_2d[NEWT_DIM];
  FTYPE QdotB,Bcon[NDIM],Qcov[NDIM],Qcon[NDIM],Qsq,Qtcon[NDIM];
...
//  fflush(stdout);
  ...
  retval = general_newton_raphson( x_2d, NEWT_DIM, func_vsq ) ;  
...
//-fast  p_tmp  = pressure_W_vsq( W, vsq );
//-fast  dPdW   = dpdW_calc_vsq( W, vsq );
//-fast  dPdvsq = dpdvsq_calc( W, vsq );
