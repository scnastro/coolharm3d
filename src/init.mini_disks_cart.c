...
/* INIT_ALL_PRIMS FOR THE CIRCUMBINARY NOT YET IMPLEMENTED!!!!!!!! */
#include "decs.h"
#include "metric.h"
#if( USEMPI )
#include "mpi.h"
...
#define INITIALIZE_MINI_DISKS (0)
...
static double PatchXlength, PatchYlength, PatchZlength;
...
void init_base(void)
{
  ...
  TRACE_BEG;
	...
	**************************************************************************/
	...
	GridLength[1] = PatchXlength;
	GridLength[2] = PatchYlength;
	GridLength[3] = PatchZlength;
#else
    --only-support-identity-coords
#endif
	/**************************************************************************
...
#if( COORD_TYPE_CHOICE == COORD_IDENTITY )
	startx[1] = -0.5 * GridLength[1];
	startx[2] = -0.5 * GridLength[2];
	startx[3] = -0.5 * GridLength[3];
#endif
...
void init_data(void) 
{
  ...
  *******************************************************************************/
  ...
  /*******************************************************************************
     Set up prim var arrays: --for circumbinary (not needed for cartesian patch)
  *******************************************************************************/
...
#if( TOP_TYPE_CHOICE == TOP_CARTESIAN )
  n_within_horizon = 0;
  PatchXlength = 1.8 * initial_bbh_separation;
  PatchYlength = 0.8 * initial_bbh_separation;
#elif( TOP_TYPE_CHOICE == TOP_TYPE_SPHERICAL )
...
void init_mini_disks(int i, int j, int k, struct of_coord *coords, double *pr)
{
  ...
  x = coords->x[XX]; y  = coords->x[YY]; z  = coords->x[ZZ];
  r  = sqrt(x*x + y*y + z*z);
  th = acos(z/r);
  ph = atan2(y,x);
  /* Get BH Traj Data */ 
  ...
  if( in_mini_disk ) {    
    ...
    transform_rank1con2(coords->dxp_dx,ucon); // transform from (x,y,z) to (x1,x2,x3)
    get_geometry(i,j,k,CENT,ncurr,geom); /* get (x1,x2,x3) metric,   */ 
...
#define X2_INTEGRATION_CODE_BLOCK_MINI                                                                       \
  {                                                                                                          \
    if( (0 <= j) && (j < N2TOT_MINI) )  {				                                     \
      ...
      lambda = xsol[0];                                                                                      \
      if( retval ) {                                                                                         \
	fprintf(stderr,"init.mini_disks_cart.c: create_disk_solutions():  ");                                \
