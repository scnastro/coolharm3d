...
#define VERT_FIELD           (2)
#define FIELD_CONFIG_CHOICE  (VERT_FIELD)
...
void set_special_coord(void)
{
  ...
  Rout     = 500.;         /* Radial extent of the grid           */
 ...
 //  th_cutout = 0.045 * M_PI;     /* Angular size to excise from the axis */
  th_cutout = SMALL;     /* Angular size to excise from the axis */
...
void init_prim(int i, int j, int k, struct of_coord *coords, double *pr)
{
  ...
  **********************************************************************************************/  
  ...
  if( first_call ) { 
    ...
    rin      = 30.;     /* Radius of inner edge of the disk ?? */
    rpmax    = 40.;     /* Radius of the pressure maximum ?? */
    rout     = 0.0;     /* Not used */
...
//    fprintf(stdout,"ucon1 = %26.20e %26.20e %26.20e %26.20e \n",ucon[0],ucon[1],ucon[2],ucon[3]); 
    ...
    if( coords->x[RR] > rdiskout ) { rdiskout = coords->x[RR]; }
  }
  else { 
    PLOOP pr[l] = 0.;
  }
...
void set_mag_field( void )
{
...
#elif( FIELD_CONFIG_CHOICE ==     VERT_FIELD )
  fprintf(stdout,"set_mag_field():  Using VERT_FIELD %d \n",FIELD_CONFIG_CHOICE); 
#else 
  ...
  *************************************************************************************/
  ...
  for(i=N1S; i<=(N1E+1); i++)  for(j=N2S; j<=(N2E+1); j++) { 
    ...
    F[i][j][0][0][0] = ( rho_avg > rho_cut ) ?  Aphi  :  0. ;   /* Truncate A_phi at zero so that field lines lie within disk */
    ...
    xin = 35.;   xout = 55. ; 
    get_coord(i,j,k,CORN,ncurr,coords);
    xx = coords->x[RR] * sin(coords->x[TH]);
    if(      xx < xin  ) {  Aphi = xin  ; } 
    else if( xx > xout ) {  Aphi = xout ; } 
    else                 {  Aphi = xx   ; } 
    F[i][j][0][0][0] = Aphi ; 
#endif
  ...
  *************************************************************************************/
  ...
  N1_LOOP  N2_LOOP { 
    ...
    /* only integrate over the disk */
    if( fabs(p[i][j][0][U3]) >= 1e-20 ) { 
...
//		x[RR],*utcov, f_l, *l_ang, lambda, h, Omega); 
//      fprintf(stdout,"equator-values: myid r lambda l h = %6d %26.16e %26.16e %26.16e %26.16e \n",
//	      myid,x[RR],lambda,*l_ang,h); 
//      fflush(stdout);
//    }
