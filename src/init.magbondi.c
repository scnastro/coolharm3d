...
/***********************************************************************************
  ...
  ---------
   ...
   Magnetized Bondi Solution (radial, spherically symmetric  magnetic field ) 
...
//	cour = 0.4 ;
	...
	**************************************************************************/
	GridLength[0] = 61. ;         /* Length of X0 dimension (evolution period) */  
...
int init_dsandvels(int i, int j, int k, double *xp, double *pr)
{
  ...
  double gcov[NDIM][NDIM], ucon_ks[NDIM], ucon_bl[NDIM];
    int gp_bak, nall;
  double Br, Bcon[NDIM], alpha, gdet_ks, xp_0[NDIM], x_0[NDIM];
  static double gdet_ks_0;
  ...
  /* Set up radial component (in KS), transform to numerical coordinates and make assignment */
  // ks_gdet_func( x, &gdet_ks );
  // Br = 1.e-1;
  // Bcon[TT] = Bcon[TH] = Bcon[PH] = 0.;
  // Bcon[RR] = Br * alpha * gdet_ks_0 / gdet_ks ;
  // transform_rank1con( x, xp, Bcon ); 
  // alpha = 1./sqrt(-gcon[i][j][CENT][TT][TT]); 
  // pr[ B1] = Bcon[1] / alpha;
  // pr[ B2] = Bcon[2] / alpha;
  // pr[ B3] = Bcon[3] / alpha;


  // pr[B1] = 1.e0 * gdet[NG][j][CENT] / gdet[i][j][CENT] ;
  // pr[B2] = 0.;
  // pr[B3] = 0.;
...
static void set_mag_field( void ) 
{
  ...
  extern  void curl_of_A( double A[][N2TOT][N3TOT][NP] , double prim[][N2TOT][N3TOT][NP] ) ;
