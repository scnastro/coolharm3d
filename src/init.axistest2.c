...
/***********************************************************************************
 ...
 Unique test to check the boundary conditions along the axis in spherical coordinates.
  This version of the test is the "test solution" and is done in spherical coordinates 
  (r,th).  The initial data is an azimuthally-symmetric Gaussian density and pressure 
  profile initially at rest. The pressure makes it expand outward and should reflect 
  through the axis symmetrically.  Since we are most interested in the U2/B2 
  boundary conditions at the axis, we need to compare the equatorial radial component 
  to the x component from the init.axistest1.c evolution (modulo some geometric factors). 
...
void init_base(void)
{
	...
	GridLength[2] = M_PI ;        /* Length of X2 dimension */ 
	GridLength[3] = M_PI/4. ;        /* Length of X3 dimension */ 
	...
	startx[1] = NG*dx[1] + 1.e-4;             /* Set the Physical Minimum boundary  */
