...
Komissarov's cylindrical explosion .  Run with 200x200
...
void init_base(void)
{
	...
	t = 0. ;                     /* Initial time */ 
	GridLength[0] = 4. ;         /* Length of X0 dimension (evolution period) */ 
	GridLength[1] = 12. ;        /* Length of X1 dimension */ 
	GridLength[2] = 12. ;        /* Length of X2 dimension */ 
	GridLength[3] = 12. ;        /* Length of X3 dimension */ 
	...
	startx[1] = -6.;               /* Set the Physical Minimum boundary  */
	startx[2] = -6.;               /* Set the Physical Minimum boundary  */
	startx[3] = -6.;               /* Set the Physical Minimum boundary  */
...
void init_data(void) 
{
  ...
  LOOP {
    ...
    p[i][j][k][B2] = 1.e-1 ;
    p[i][j][k][B3] = 0. ;
    p[i][j][k][RHO] = fr(r,1.e-4,1.e-2) ;
    p[i][j][k][UU] = fr(r,3.e-5,1.)/(gam - 1.) ;
  }
